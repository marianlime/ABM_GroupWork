...
Usage
-----
    python Analysis/comparison_runs.py
...
from main import run_experiment  # triggers Analysis/analysis.py (forces Agg backend)
...
POPULATION_SWEEP = [(20, 80),(30,70), (40,60), (50,50),(60,40),(70,30),(80,20)]
...
PARAM_SUBPLOTS = [("Qty Aggression",[("mean_qty_aggression",None)]), ("Signal Aggression", [("mean_signal_aggression",None)]),("Info Param (informed)", [("mean_info_param_parameterised_informed", None)])]
STD_SUBPLOTS = [("Qty Aggression σ",[("std_qty_aggression",None)]),("Signal Aggression σ", [("std_signal_aggression", None)]),("Info Param σ",[("std_info_param_parameterised_informed", None)])]

def _run_single_worker(args: tuple) -> tuple: #Input (label, overrides) - output (label, df) where df is generated by run_experiment and filtered to contain only required columns for plotting.
    
    # Runs single worker process for a single run, returning the label and filtered DataFrame for that run. 
    # DataFrame is filtered to contain only columns required for plotting, and all columns are cast to float.

    label, overrides = args #Unpacks input argument
    result = run_experiment( config_overrides=overrides, progress_callback=None, run_analysis=False, disable_database_writes=True) #Runs the experiment

    df = result["generation_counts_df"].reset_index(drop=True)

    #Collects required DataFrame names for columns
    needed_cols = {"generation", WEALTH_INFORMED_COLUMN, WEALTH_ZI_COLUMN}
    for subplots in [PARAM_SUBPLOTS, STD_SUBPLOTS]:
        for _, series_specs in subplots:
            for col, _ in series_specs:
                needed_cols.add(col)
                
    keep_columns = list(needed_cols.intersection(df.columns))
...
def _sweep_colors(n: int) -> list:
    colour_map = plt.get_cmap(SWEEP_COLORMAP)
    return [colour_map(i / max(n - 1, 1)) for i in range(n)]

def _build_subplot_figure(title: str, runs: list, subplot_specs: list, ylabel: str) -> plt.Figure:
...
def _build_wealth_figure(title: str, runs: list) -> plt.Figure:
    ...
    for run_idx, (run_label, df) in enumerate(runs):
        color = colors[run_idx]
        if WEALTH_INFORMED_COLUMN not in df.columns or WEALTH_ZI_COLUMN not in df.columns:
            continue
        diff   = df[WEALTH_INFORMED_COLUMN].astype(float) - df[WEALTH_ZI_COLUMN].astype(float)
        smooth = _smooth(diff)
        gens   = df["generation"].values
...
def run_population_sweep() -> list:
    ...
    for n_informed, n_zi in POPULATION_SWEEP:
        ...
        overrides = {"n_parameterised_agents": n_informed, "n_zi_agents": n_zi, "n_generations":N_GENERATIONS,"n_rounds":N_ROUNDS,"GBM_drift":FIXED_DRIFT, "GBM_volatility": FIXED_VOLATILITY}
...
def run_drift_sweep() -> list:
    ...
    for drift in DRIFT_SWEEP:
        ...
        overrides = {"n_parameterised_agents": DEFAULT_N_INFORMED,"n_zi_agents":DEFAULT_N_ZI,"n_generations":N_GENERATIONS,"n_rounds":N_ROUNDS,"GBM_drift":drift,"GBM_volatility": FIXED_VOLATILITY}
        run_args.append((label, overrides))
    return _run_parallel(run_args)
...
def run_volatility_sweep() -> list:
    ...
    for vol in VOLATILITY_SWEEP:
        label = f"Vol {vol:.2f}"
        overrides = {"n_parameterised_agents": DEFAULT_N_INFORMED,"n_zi_agents":DEFAULT_N_ZI,"n_generations":N_GENERATIONS,"n_rounds":N_ROUNDS,"GBM_drift":FIXED_DRIFT,"GBM_volatility": vol}
        run_args.append((label, overrides))
    return _run_parallel(run_args)
...
_SWEEP_REGISTRY = {
    "population": (run_population_sweep, f"Population Sweep  (drift={FIXED_DRIFT}, vol={FIXED_VOLATILITY}, total={TOTAL_AGENTS})",),
    "drift": (run_drift_sweep,f"GBM Drift Sweep  (vol={FIXED_VOLATILITY}, {DEFAULT_N_INFORMED} parametrised, {DEFAULT_N_ZI} ZI)",),
    "volatility": (run_volatility_sweep, f"GBM Volatility Sweep  (drift={FIXED_DRIFT}, {DEFAULT_N_INFORMED} parametrised, {DEFAULT_N_ZI} ZI)",),
}
